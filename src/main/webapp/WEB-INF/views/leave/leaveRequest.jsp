...
<section>
	<form:form action="${pageContext.request.contextPath}/leave/request/submit" 
		method="POST" 
		id="leaveFrm"
		enctype="multipart/form-data">
		<p>잔여 연차 : ${leaveCnt}일</p>
		<hr>
		<div class="form-group leave-div">
			<label for="exampleFormControlInput1">신청자</label>
			<input type="text" class="form-control" id="exampleFormControlInput1" value="${applicantName}" readonly>
		</div>
		...
		<select class="custom-select" name="category">
			...
			<option value="6">출산</option>
			<option value="7">무급</option>
		</select>

		<div class="form-check form-check-inline leave-div">
			<input class="form-check-input" type="radio" name="lvDuration" id="inlineRadio1" value="M">
			<label class="form-check-label" for="inlineRadio1">오전반차</label>
		</div>

		<div class="form-check form-check-inline leave-div">
			<input class="form-check-input" type="radio" name="lvDuration" id="inlineRadio2" value="A">
			<label class="form-check-label" for="inlineRadio2">오후반차</label>
		</div>

		<div class="form-check form-check-inline leave-div">
			<input class="form-check-input" type="radio" name="lvDuration" id="inlineRadio2" value="E" selected>
			<label class="form-check-label" for="inlineRadio2">일차</label>
		</div>
		
		<div class="form-check selected-div">
			<input type="hidden" name="lvDates" id="selected-dates" />
		</div>
		...
		<div class="input-group mb-3 leave-div">
			<div class="custom-file">
				<input type="file" class="custom-file-input" id="inputGroupFile01" name="lvFile" aria-describedby="inputGroupFileAddon01">
				<label class="custom-file-label" for="inputGroupFile01">Choose file</label>
			</div>
		</div>
		<div class="form-group">
			<input type="hidden" name="applicant" value="<sec:authentication property='principal.id'/>" />
		...
		<div class="leave-request-btn-div">
			...
			<button type="button" class="leave-request-btn btn btn-primary btn-lg" id="send-btn" onclick="leaveFrmSubmit();">보내기</button>
		</div>
	</form:form>
</section>
...
$(function () {
	$('#datepicker').datepicker({
	    ...
	    updateArr($('#datepicker').datepicker('getFormattedDate'));
	});
